...
import { ArrowLeft, Swords, Shield, Zap, Heart, Wind, Activity, Star, Dna, Egg, Sparkles, Edit3, Gem, X } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm p-6 animate-in fade-in zoom-in-95">
            <div className="bg-slate-900 border border-white/10 w-full max-w-xs rounded-[32px] p-6 shadow-2xl relative overflow-hidden">
                ...
                <div className="text-center mb-6">
                    <div className="w-12 h-12 bg-indigo-500/20 text-indigo-400 rounded-2xl flex items-center justify-center mx-auto mb-3 border border-indigo-500/30">
                ...
                <input 
                    ...
                    className="w-full bg-slate-950 border border-white/10 rounded-xl p-4 text-center font-bold text-white mb-6 focus:border-indigo-500 outline-none transition-colors text-lg"
                    autoFocus
                ...
                <button 
                    ...
                    className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 font-black text-white flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
                >
                    <span>{cost}</span> <Gem className="w-4 h-4 fill-current text-pink-300" /> UMBENENNEN
                </button>
            </div>
        </div>
    );
...
export default function PetDetailScreen({ pet, onBack, onRenamePet }) { // onRenamePet prop
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300 relative bg-slate-950">
        ...
        <div className="relative z-10 flex items-center justify-between p-4">
            <button onClick={onBack} className="p-2 bg-black/20 hover:bg-black/40 backdrop-blur-md text-white rounded-full transition-all active:scale-95 border border-white/10">
        ...
        <div className="relative z-10 flex flex-col items-center -mt-2">
            <div className="relative">
                ...
                <div className="absolute bottom-0 right-0 bg-slate-900 text-white text-xs font-black px-2.5 py-1 rounded-lg border border-white/20 shadow-lg">
                    Lvl {pet.level}
                </div>
            </div>
            
            <div className="mt-6 text-center flex items-center gap-2">
                <h1 className={`text-3xl font-black text-white drop-shadow-lg`}>{pet.name}</h1>
                <button 
                    onClick={() => setShowRenameModal(true)}
                    className="p-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors border border-white/5"
                >
            ...
            <div className="flex items-center gap-2 mt-2">
                ...
                <div className={`flex items-center gap-1 ${type.color} text-[10px] font-black uppercase`}>
        ...
        {/* --- STATS CARD --- */}
        <div className="flex-1 bg-slate-900/80 backdrop-blur-xl mt-8 rounded-t-[40px] border-t border-white/10 p-6 overflow-y-auto relative">
            ...
            <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 p-5 rounded-3xl border border-indigo-500/20 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-2">
                        <div className="p-1.5 bg-indigo-500 rounded-lg text-white shadow-lg shadow-indigo-500/20">
                            <Zap className="w-3.5 h-3.5 fill-current" />
                        </div>
                        <span className="text-xs font-black text-indigo-300 uppercase tracking-widest">Spezialfähigkeit</span>
                    </div>

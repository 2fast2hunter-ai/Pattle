    ...
    return (
      <div className="pt-4 space-y-6">
        ...
        <div className="bg-slate-800 rounded-2xl overflow-hidden border border-white/5">
          <SettingRow icon={settings.music ? Volume2 : VolumeX} label="Musik" active={settings.music} onToggle={() => toggle('music')} />
          <SettingRow icon={settings.sfx ? Zap : VolumeX} label="Soundeffekte" active={settings.sfx} onToggle={() => toggle('sfx')} />
        </div>
        
        <button onClick={onLogout} className="w-full bg-red-500/10 text-red-400 border border-red-500/20 p-4 rounded-xl flex items-center justify-center gap-2 font-bold"><LogOut className="w-5 h-5" /> Abmelden</button>
      </div>
    );
}
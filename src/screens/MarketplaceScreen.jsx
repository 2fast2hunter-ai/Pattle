            ...
            <div className="bg-slate-900 border border-white/10 w-full max-w-sm rounded-3xl p-0 relative overflow-hidden flex flex-col max-h-[85vh]">
                ...
                <div className="p-5 flex-1 overflow-y-auto scrollbar-hide">
                    <div className="flex justify-between items-start mb-4">
                        <div><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold text-slate-500 uppercase">{speciesInfo.label}</span><span className={`text-[10px] font-bold ${typeInfo.color} uppercase border border-white/10 px-1.5 rounded`}>{typeInfo.label}</span></div><h2 className={`text-2xl font-black ${rarityInfo.color}`}>{pet.name}</h2></div>
...
export default function MarketplaceScreen({ user, listings, onBack, onBuy, onSell, myPets }) {
    ...
    const [sellQuantity, setSellQuantity] = useState(1); // NEU: Menge
    ...
    // STACKING LOGIK (nur für Verkauf)
    const rawSellList = myPets
    ...
    const handleSellSubmit = () => {
        ...
        const unitPrice = parseInt(sellPrice);
        
        if (selectedForSale.isStack) {
            // BULK SELL: Wir übergeben das Array der Pets an onSell
            const qty = Math.max(1, Math.min(sellQuantity, selectedForSale.count));
            const petsToSell = selectedForSale.pets.slice(0, qty);
            const totalPrice = unitPrice * qty; // Gesamtpreis für das Bundle
            
            onSell(petsToSell, totalPrice);
        } else {
            // EINZEL SELL
            onSell([selectedForSale], unitPrice);
        }
    ...
    const MarketCard = ({ pet, price, seller, isSelected, onClickCard, onClickAction, quantity }) => {
        ...
        const type = isEgg ? null : TYPES[pet.type];
        const isStack = pet.isStack || quantity > 1; // Check ob Stapel (Verkauf oder Kauf)
        const displayCount = quantity || pet.count || 1; // Anzahl anzeigen
                 ...
                 <div className="p-3 flex items-center gap-4 relative">
                    ...
                    <div className="flex-1 flex items-center gap-4 cursor-pointer" onClick={() => { if (!isEgg && onClickCard) onClickCard(); }}>
                        <div className="relative">
                            ...
                            {/* STACK / EGG BADGE */}
                            {isStack ? (
                                <div className="absolute -top-1 -right-1 bg-white text-slate-900 text-[10px] font-black px-1.5 py-0.5 rounded shadow-lg z-20">x{displayCount}</div>
                            ) : (
                                isEgg && <div className="absolute -bottom-1 -right-1 bg-slate-950 text-slate-400 text-[10px] font-black px-1.5 py-0.5 rounded border border-white/10 z-20">EI</div>
                            )}
            ...
            {isSidebarOpen && (
                <div className="fixed inset-0 z-50 flex">
                    ...
                    <div className="relative w-4/5 max-w-xs bg-slate-900 h-full shadow-2xl p-5 flex flex-col gap-6 border-r border-white/10 animate-in slide-in-from-left duration-300">
                        {/* ... Sidebar Content (Kurzfassung für Übersicht) ... */}
                        ...
                        {buyList.length === 0 ? (
                            ...
                            buyList.map(listing => (
                                <MarketCard 
                                    ...
                                    quantity={listing.quantity} // WICHTIG: Menge übergeben für Badge
                                        ...
                                        {isSelected && (
                                            <div className="p-3 mt-[-4px] mx-0.5 border-x border-b border-green-500/30 rounded-b-2xl bg-slate-900/80 animate-in slide-in-from-top-2 mb-3">
                                                ...
                                                {item.isStack && item.count > 1 && (
                                                    <div className="flex items-center justify-between mb-3 bg-black/30 p-2 rounded-xl">
                                                        <span className="text-xs font-bold text-slate-400 ml-2">Menge:</span>
                                                        <div className="flex items-center gap-3">
                                                            <button onClick={() => setSellQuantity(Math.max(1, sellQuantity - 1))} className="p-1 bg-slate-700 rounded hover:bg-slate-600"><Minus className="w-4 h-4 text-white" /></button>
                                                            <span className="font-black text-white w-6 text-center">{sellQuantity}</span>
                                                            <button onClick={() => setSellQuantity(Math.min(item.count, sellQuantity + 1))} className="p-1 bg-slate-700 rounded hover:bg-slate-600"><Plus className="w-4 h-4 text-white" /></button>
                                                        </div>
                                                    </div>
                                                )}

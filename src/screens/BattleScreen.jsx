...
const FloatingText = ({ text, color }) => (
  <div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-16 z-[100] pointer-events-none animate-bounce`}>
      <span className={`text-3xl font-black ${color} drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] tracking-wider`} 
...
function BattleUnit({ pet, isEnemy, isActive, isHit, damageText }) {
  ...
  const attackAnim = isActive ? (isEnemy ? '-translate-x-12 scale-105 rotate-[-2deg]' : 'translate-x-12 scale-105 rotate-[2deg]') : '';
  const hitAnim = isHit ? (isEnemy ? 'translate-x-4 brightness-150' : '-translate-x-4 brightness-150') : '';
  ...
  return (
   <div className={`flex flex-col ${isEnemy ? 'items-end' : 'items-start'} relative group w-full max-w-[280px]`}>
      ...
      <div className={`relative z-20 transition-all duration-300 ease-out ${attackAnim} ${hitAnim} mb-2`}>
        ...
        <div className="relative">
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 ${typeInfo.bg} opacity-30 blur-[40px] rounded-full`}></div>
            <div className="drop-shadow-xl filter transition-transform">
                <PetAvatar pet={pet} className="w-28 h-28" />
            </div>
        </div>
        {pet.currentCd > 0 && (
            <div className="absolute -top-2 -right-2 bg-black/70 backdrop-blur-md text-white text-[10px] font-black px-2 py-0.5 rounded-full border border-white/20 flex items-center gap-1 shadow-lg">
      ...
      <div className={`
          ...
          bg-slate-900/70 backdrop-blur-md border border-white/10 p-2.5 rounded-xl shadow-xl
          ${isActive ? 'ring-1 ring-white/30 z-30' : 'opacity-95'}
      `}>
          <div className="flex justify-between items-center mb-2">
              <div className="flex flex-col overflow-hidden">
                  <span className={`font-black text-sm truncate ${isEnemy ? 'text-red-200' : 'text-indigo-200'}`}>
                      {pet.name}
                  </span>
              ...
              <div className={`
                  flex items-center gap-1 px-1.5 py-0.5 rounded-md border border-white/5 shadow-sm
              ...
              `}>
                  ...
                  <span className="text-[8px] font-black text-white uppercase">{typeInfo.label}</span>
              </div>
          </div>
          <div className="relative w-full h-3 bg-slate-950 rounded-full overflow-hidden mb-1 shadow-inner border border-white/5">
              <div 
          ...
          <div className="flex justify-end mb-1">
              <span className="text-[9px] font-mono font-bold text-slate-500">{pet.hp}/{pet.maxHp}</span>
          </div>
...
const StatBox = ({ icon: Icon, val, color, bg }) => (
    <div className={`flex flex-col items-center justify-center py-1 rounded ${bg} border border-white/5`}>
...
export default function BattleScreen({ battleState, setBattleState, onWin, onLose }) {
  ...
  if (isOver) {
    ...
    // UPDATE: Münz-Belohnung auf 5 bei Niederlage gesetzt (statt 0)
    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden animate-in zoom-in duration-500 z-50 bg-slate-900">
          ...
          <div className="relative z-10 flex flex-col h-full p-6">
              ...
              {/* Header: Sieg/Niederlage */}
              <div className="flex flex-col items-center mb-6 mt-4">
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-3 shadow-[0_0_40px_rgba(0,0,0,0.5)] border-4 border-white/10 ${won ? 'bg-gradient-to-br from-yellow-400 to-amber-600' : 'bg-gradient-to-br from-red-500 to-red-800'}`}>
                      {won ? <Trophy className="w-10 h-10 text-white drop-shadow-md" /> : <Skull className="w-10 h-10 text-white drop-shadow-md" />}
                  </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-hide mb-6">
                  ...
                  <div className="space-y-3">
                      {myTeam.map((pet) => {
                          // Berechnung für Visualisierung (vereinfacht, da wir das echte Level-Up im Backend machen)
                          const currentPercent = (pet.xp / pet.maxXp) * 100;
                          const gainPercent = (petXpGain / pet.maxXp) * 100;
                          // Begrenzen auf 100% für die Anzeige
                          const totalPercent = Math.min(100, currentPercent + gainPercent);

                          return (
                              <div key={pet.id} className="bg-slate-800/40 border border-white/5 p-3 rounded-xl flex items-center gap-4">
                                  <div className="relative">
                                      <PetAvatar pet={pet} className="w-12 h-12 drop-shadow-md" />
                                      <div className="absolute -bottom-1 -right-1 bg-slate-900 text-[9px] font-bold px-1.5 rounded border border-white/10 text-white">Lvl {pet.level}</div>
                                  </div>
                                  
                                  <div className="flex-1">
                                      <div className="flex justify-between items-end mb-1.5">
                                          <span className="font-bold text-sm text-slate-200">{pet.name}</span>
                                          <span className="text-[10px] font-mono font-bold text-green-400 flex items-center gap-0.5">
                                              <ArrowUp className="w-3 h-3" /> {petXpGain} XP
                                          </span>
                                      </div>
                                      
                                      {/* XP Bar mit "Gain" Indikator */}
                                      <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden relative border border-white/5">
                                          ...
                                          {/* Zuwachs (Hell) - Startet bei Basis XP */}
                                      ...
                                      <div className="flex justify-end mt-0.5">
                                           <span className="text-[8px] text-slate-500 font-mono">{pet.xp + petXpGain} / {pet.maxXp} XP</span>
                                      </div>
                                  </div>
                              </div>
                          );
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden bg-slate-950">
      ...
      <div className="flex-1 flex flex-col relative z-10 p-2">
          ...
          <div className="flex-1 flex flex-col items-end justify-center pr-2 pt-24 relative">
      ...
      <div className="h-32 relative shrink-0 z-20 border-t border-white/5 bg-slate-950/90 backdrop-blur-md">
          <div className="absolute top-0 left-0 w-full h-4 bg-gradient-to-b from-black/20 to-transparent pointer-events-none"></div>
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5 font-mono text-[10px] scrollbar-hide h-full">

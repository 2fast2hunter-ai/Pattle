...
const FloatingText = ({ text, color }) => (
  <div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-16 z-[100] pointer-events-none animate-bounce`}>
      <span className={`text-2xl sm:text-3xl font-black ${color} drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] tracking-wider`} style={{ WebkitTextStroke: '1px rgba(0,0,0,0.5)' }}>{text}</span>
  </div>
);

const TeamDots = ({ team, currentIndex, isEnemy }) => (
  <div className={`flex gap-1.5 mb-2 ${isEnemy ? 'justify-end' : 'justify-start'}`}>
    {team.map((p, i) => (<div key={i} className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-500 border border-white/10 ${i === currentIndex ? (isEnemy ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)] scale-125' : 'bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)] scale-125') : (p.hp <= 0 ? 'bg-slate-800' : 'bg-slate-600')}`} />))}
  </div>
);
...
function BattleUnit({ pet, isEnemy, isActive, isHit, damageText }) {
  ...
  const getHpColor = (pct) => { if (pct > 50) return 'bg-gradient-to-r from-emerald-500 to-green-400'; if (pct > 20) return 'bg-gradient-to-r from-yellow-500 to-orange-400'; return 'bg-gradient-to-r from-red-600 to-red-500 animate-pulse'; };
  ...
  return (
   <div className={`flex flex-col ${isEnemy ? 'items-end' : 'items-start'} relative group w-full max-w-[220px] sm:max-w-[280px]`}>
      <div className={`relative z-20 transition-all duration-300 ease-out ${attackAnim} ${hitAnim} mb-2`}>
        ...
        <div className="relative"><div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 sm:w-20 sm:h-20 ${typeInfo.bg} opacity-30 blur-[30px] sm:blur-[40px] rounded-full`}></div><div className="drop-shadow-xl filter transition-transform"><div className="w-24 h-24 sm:w-28 sm:h-28 flex items-center justify-center"><PetAvatar pet={pet} className="w-full h-full" /></div></div></div>
        {pet.currentCd > 0 && (<div className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 bg-black/70 backdrop-blur-md text-white text-[9px] sm:text-[10px] font-black px-1.5 py-0.5 rounded-full border border-white/20 flex items-center gap-1 shadow-lg"><Timer className="w-3 h-3 text-slate-400" /><span>{pet.currentCd}</span></div>)}
      </div>
      <div className={`relative w-full transition-all duration-500 bg-slate-900/70 backdrop-blur-md border border-white/10 p-2 sm:p-2.5 rounded-xl shadow-xl ${isActive ? 'ring-1 ring-white/30 z-30' : 'opacity-95'}`}>
          <div className="flex justify-between items-center mb-1.5"><div className="flex flex-col overflow-hidden"><span className={`font-black text-xs sm:text-sm truncate ${isEnemy ? 'text-red-200' : 'text-indigo-200'}`}>{pet.name}</span><span className="text-[8px] sm:text-[9px] text-slate-500 font-bold uppercase tracking-wider">Lvl {pet.level}</span></div><div className={`flex items-center gap-1 px-1 py-0.5 rounded-md border border-white/5 shadow-sm bg-gradient-to-br ${typeInfo.bg} from-black/40 to-transparent`}><div className="text-white">{React.cloneElement(typeInfo.icon, { size: 10 })}</div><span className="text-[7px] sm:text-[8px] font-black text-white uppercase">{typeInfo.label}</span></div></div>
          <div className="relative w-full h-2.5 sm:h-3 bg-slate-950 rounded-full overflow-hidden mb-1 shadow-inner border border-white/5"><div className={`absolute top-0 left-0 h-full transition-all duration-500 ease-out shadow-[0_0_5px_rgba(255,255,255,0.2)] ${getHpColor(hpPercent)}`} style={{ width: `${hpPercent}%` }}></div></div>
          <div className="flex justify-end mb-1"><span className="text-[8px] sm:text-[9px] font-mono font-bold text-slate-500">{pet.hp}/{pet.maxHp}</span></div>
          <div className="grid grid-cols-5 gap-0.5"><StatBox icon={Swords} val={pet.atk} color="text-red-400" bg="bg-red-500/5" /><StatBox icon={Shield} val={pet.def} color="text-slate-400" bg="bg-slate-500/5" /><StatBox icon={Zap} val={pet.ap} color="text-purple-400" bg="bg-purple-500/5" /><StatBox icon={Activity} val={pet.res} color="text-pink-400" bg="bg-pink-500/5" /><StatBox icon={Wind} val={pet.speed} color="text-sky-400" bg="bg-sky-500/5" /></div>
      </div>
    </div>
  );
}
const StatBox = ({ icon: Icon, val, color, bg }) => (<div className={`flex flex-col items-center justify-center py-0.5 sm:py-1 rounded ${bg} border border-white/5`}><Icon className={`w-2 h-2 sm:w-2.5 sm:h-2.5 ${color} mb-0.5`} /><span className="text-[7px] sm:text-[8px] font-black text-slate-300 leading-none">{val}</span></div>);
...
export default function BattleScreen({ battleState, setBattleState, onWin, onLose, isAutoBattle, autoBattleRemaining, onCancelAutoBattle }) {
  ...
  const { myTeam, enemyTeam, myIndex, enemyIndex, turn, log, isOver, round, isFriendly } = battleState; // isFriendly holen
  ...
  if (isOver) {
    const won = enemyTeam[enemyTeam.length - 1].hp === 0;
    const myTeamIds = myTeam.map(p => p.id);
    // Rewards (NUR WENN NICHT FREUNDLICH)
    const rewardCoins = !isFriendly && won ? 50 : (!isFriendly ? 5 : 0);
    const rewardXp = !isFriendly && won ? 50 : (!isFriendly ? 5 : 0);
    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden animate-in zoom-in duration-500 z-50 bg-slate-900">
          ...
          <div className="relative z-10 flex flex-col h-full p-6">
              ...
              {/* REWARDS ODER FREUNDSCHAFTS-MELDUNG */}
              {!isFriendly ? (
              ...
              {/* PET LISTE (Nur anzeigen wenn KEIN Freundschaftskampf ODER wenn gewonnen, optional) */}
                      ...
                      {myTeam.map((pet) => (
                          <div key={pet.id} className="bg-slate-800/40 border border-white/5 p-2 sm:p-3 rounded-xl flex items-center gap-3 sm:gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-end mb-1.5"><span className="font-bold text-xs sm:text-sm text-slate-200 truncate">{pet.name}</span></div>
              ...
              <div className="flex flex-col gap-2">
                  <button onClick={() => won ? onWin({coins: rewardCoins, xp: rewardXp}, myTeamIds) : onLose()} className={`w-full py-4 rounded-2xl font-black text-lg shadow-lg flex justify-center items-center gap-2 transition-all ${isAutoBattle ? 'bg-purple-600 text-white animate-pulse' : 'bg-white text-slate-950 hover:scale-[1.02] active:scale-95'}`}>
                      {isAutoBattle ? (<> <Repeat className="w-5 h-5" /> WEITER ({autoBattleRemaining}) </>) : (isFriendly ? "ZURÜCK" : "WEITER")}
                  </button>
                  {isAutoBattle && (<button onClick={onCancelAutoBattle} className="w-full py-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold text-xs border border-red-500/30 flex items-center justify-center gap-2 active:scale-95 transition-all"><XCircle className="w-4 h-4" /> AUTO-KAMPF ABBRECHEN</button>)}
              </div>
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden bg-slate-950">
      ...
      <div className="absolute inset-0 pointer-events-none"><div className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-indigo-900/20 blur-[100px] rounded-full"></div><div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-red-900/20 blur-[100px] rounded-full"></div><div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-5"></div></div>
      <div className="absolute top-2 left-0 w-full flex justify-center z-10"><div className="bg-black/40 backdrop-blur-sm px-4 py-1 rounded-full border border-white/5"><span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Runde {round}</span></div></div>
      <div className="flex-1 flex flex-col relative z-10 p-2">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-5 pointer-events-none select-none"><span className="text-[10rem] sm:text-[12rem] font-black italic">VS</span></div>
          <div className="flex-1 flex flex-col items-end justify-center pr-2 pt-16 sm:pt-24 relative"><TeamDots team={enemyTeam} currentIndex={enemyIndex} isEnemy={true} /><BattleUnit pet={enemyPet} isEnemy={true} isActive={turn === 'ENEMY' && animatingUnit === 'ENEMY'} isHit={hitUnit === 'ENEMY'} damageText={hitUnit === 'ENEMY' ? floatingDamage : null} /></div>
          <div className="flex-1 flex flex-col items-start justify-center pl-2 mt-[-20px]"><BattleUnit pet={myPet} isEnemy={false} isActive={turn === 'PLAYER' && animatingUnit === 'PLAYER'} isHit={hitUnit === 'PLAYER'} damageText={hitUnit === 'PLAYER' ? floatingDamage : null} /><div className="mt-2"><TeamDots team={myTeam} currentIndex={myIndex} isEnemy={false} /></div></div>
      </div>
      <div className="h-28 sm:h-32 relative shrink-0 z-20 border-t border-white/5 bg-slate-950/90 backdrop-blur-md">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5 font-mono text-[9px] sm:text-[10px] scrollbar-hide h-full">
              {[...log].reverse().map((entry, i) => {
                  ...
                  return (<div key={i} className={`${i === 0 ? 'opacity-100 text-white font-bold' : 'opacity-50 text-slate-400'} ${isDeath ? 'text-red-400 !opacity-100' : ''} ${isCrit ? 'text-yellow-400 !opacity-100' : ''}`}>{entry}</div>);
              })}
              <div ref={logEndRef} />
          </div>
      </div>

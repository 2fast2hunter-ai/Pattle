...
export default function InventoryScreen({ pets, onSelectPet, onBack, title, highlightMode }) {
  ...
  // 1. FILTER: NUR geschlüpfte Pets anzeigen!
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in relative">
        ...
        <div className="fixed inset-0 z-50 flex">
            ...
            <div className="relative w-4/5 max-w-xs bg-slate-900 h-full shadow-2xl p-5 flex flex-col gap-6 animate-in slide-in-from-left duration-300 border-r border-white/10">
                <div className="flex justify-between items-center pb-4 border-b border-white/10">
                    <h2 className="text-xl font-black text-white flex items-center gap-2"><Filter className="w-5 h-5 text-blue-400" /> FILTER</h2>
                ...
                <div className="flex-1 overflow-y-auto space-y-6 scrollbar-hide">
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 pl-10 pr-4 text-sm text-white outline-none focus:border-blue-500" />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">Sortieren</label>
                        <div className="grid grid-cols-2 gap-2">
                            {[{id:'RARITY',l:'Seltenheit'}, {id:'LEVEL',l:'Level'}, {id:'ATK',l:'Angriff'}, {id:'HP',l:'Leben'}].map(opt => (
                                <button key={opt.id} onClick={() => setSortBy(opt.id)} className={`px-3 py-2 rounded-xl text-xs font-bold border ${sortBy === opt.id ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>{opt.l}</button>
                            ))}
                        </div>
                    </div>
                    <div className="space-y-2">
                ...
                <div className="pt-4 border-t border-white/10">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="w-full py-3 rounded-xl bg-blue-600 text-white font-bold text-sm">Fertig</button>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="relative flex items-center justify-between mb-4 pt-2 px-4">
          <button onClick={() => setSidebarOpen(true)} className={`p-2 rounded-xl border transition-all relative ${activeFilterCount > 0 ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-800 border-white/10 text-slate-400'}`}>
              <Filter className="w-5 h-5" />
              {activeFilterCount > 0 && <div className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[9px] font-bold border border-slate-900">{activeFilterCount}</div>}
          </button>

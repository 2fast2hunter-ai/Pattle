...
import { Trophy, Crown, Wallet, ArrowUpRight, ArrowDownRight, Dna, ThermometerSun, X } from 'lucide-react';
import { TYPES } from '../../data/gameData';

export default function StatDetailModal({ category, data, onClose, t }) {
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white/5 p-3 rounded-2xl border border-white/10 text-center">
                                <div className="text-slate-400 text-xs font-bold uppercase">{t ? t('stat_wins') : 'Wins'}</div>
                                <div className="text-3xl font-black text-green-400">{data.wins}</div>
                            </div>
                            <div className="bg-white/5 p-3 rounded-2xl border border-white/10 text-center">
                                <div className="text-slate-400 text-xs font-bold uppercase">{t ? t('stat_losses') : 'Losses'}</div>
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between text-xs font-bold text-slate-400">
                                <span>{t ? t('stat_winrate') : 'Win Rate'}</span>
                        ...
                        <div className="bg-indigo-900/20 p-4 rounded-2xl border border-indigo-500/30 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-indigo-200 text-xs font-bold uppercase">{t ? t('stat_current_rating') : 'Current Rating'}</div>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white/5 p-3 rounded-2xl border border-white/10">
                                <div className="text-slate-400 text-xs font-bold uppercase">{t ? t('stat_total') : 'Total'}</div>
                                <div className="text-2xl font-black text-white">{data.totalPets} <span className="text-sm text-slate-500">Pets</span></div>
                            </div>
                            <div className="bg-white/5 p-3 rounded-2xl border border-white/10">
                                <div className="text-slate-400 text-xs font-bold uppercase">{t ? t('stat_top_level') : 'Top Level'}</div>
                                <div className="text-2xl font-black text-yellow-400">{data.highestLevel}</div>
                            </div>
                        </div>

                        {data.speciesProgress && (
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between text-xs font-bold mb-1">
                                        <span className="text-slate-300">{t ? t('stat_species_found') : 'Species Found'}</span>
                                ...
                                <div>
                                    <div className="flex justify-between text-xs font-bold mb-1">
                                        <span className="text-slate-300">{t ? t('stat_types_mastered') : 'Elements Mastered'}</span>
                        ...
                        <div className="space-y-2 pt-2 border-t border-white/10">
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">{t ? t('stat_rarity_dist') : 'Rarity Distribution'}</h4>
                                ...
                                r.count > 0 && (
                                    <div key={r.label} className="flex items-center gap-2 text-xs">
                                        ...
                                        <div className="flex-1 h-2 bg-slate-950 rounded-full overflow-hidden">
                                            <div style={{width: `${(r.count / (data.totalPets || data.inventoryBredCount)) * 100}%`}} className={`h-full ${r.bg}`}></div>
                                        </div>
                    ...
                    <div className="space-y-4">
                         <div className="bg-gradient-to-r from-yellow-900/20 to-amber-900/20 p-4 rounded-2xl border border-amber-500/30">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-amber-200 text-xs font-bold uppercase">{t ? t('stat_total_wealth') : 'Total Wealth'}</span>
                                <Wallet className="w-5 h-5 text-amber-400" />
                            </div>
                            <div className="text-3xl font-black text-white">{data.coins} <span className="text-sm text-amber-400">{t ? t('reward_coins') : 'Coins'}</span></div>
                        </div>

                        {data.marketEarned !== undefined && (
                            <div className="space-y-2">
                                <div className="flex justify-between items-center p-3 bg-slate-800 rounded-xl border border-white/5">
                                    <span className="text-slate-400 text-xs font-bold">{t ? t('stat_market_earnings') : 'Market Earnings'}</span>
                                ...
                                <div className="flex justify-between items-center p-3 bg-slate-800 rounded-xl border border-white/5">
                                    <span className="text-slate-400 text-xs font-bold">{t ? t('stat_market_spent') : 'Market Spent'}</span>
                                ...
                                <div className="flex justify-between items-center p-3 bg-slate-800 rounded-xl border border-white/5 pt-2 mt-2 border-t-white/10">
                                    <span className="text-slate-200 text-xs font-bold uppercase">{t ? t('stat_balance') : 'Balance'}</span>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="aspect-square bg-pink-900/20 rounded-3xl border border-pink-500/30 flex flex-col items-center justify-center gap-2">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-[10px] text-pink-200 uppercase font-bold">{t ? t('stat_total_bred') : 'Total Bred'}</div>
                                </div>
                            </div>
                            <div className="aspect-square bg-emerald-900/20 rounded-3xl border border-emerald-500/30 flex flex-col items-center justify-center gap-2">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-[10px] text-emerald-200 uppercase font-bold">{t ? t('stat_total_hatched') : 'Total Hatched'}</div>
                                </div>
                            </div>
                        </div>

                        {data.inventoryBredCount !== undefined && (
                            <div className="space-y-2 pt-2 border-t border-white/10">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">{t ? t('stat_achievements_owned', { count: data.inventoryBredCount }) : `Achievements Owned (${data.inventoryBredCount})`}</h4>

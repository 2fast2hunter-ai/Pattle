...
const watchAdForReward = async () => {
        // 1. Sicherheitscheck: Ist ein User eingeloggt?
        if (!user) return; 
        
        const reward = SHOP_ITEMS.AD_REWARD_ENERGY;
        const maxEnergy = getMaxEnergy(user.level);
        
        // 2. WICHTIG: Wir berechnen die AKTUELLE Energie (DB-Wert + verstrichene Zeit)
        // Das verhindert, dass Energie, die während der Wartezeit regeneriert wurde, überschrieben wird.
        const realEnergy = calculateCurrentEnergy(user);
    
        // 3. Prüfung: Ist die Energie schon voll?
        if (realEnergy >= maxEnergy) {
        ...
        // Test-ID für Android (Für Release bitte durch echte ID ersetzen)
        const adUnitId = 'ca-app-pub-3940256099942544/5224354917'; 
    
        // 4. AdMob: Video vorbereiten (Laden)
        try {
        ...
        // 5. AdMob: Video anzeigen
        try {
            const result = await AdMob.showRewardVideoAd();
    
            // Prüfen, ob der Nutzer das Video wirklich zu Ende geschaut hat
            if (result && result.amount) { 
                
                // Neue Energie berechnen: Aktuelle (reale) Energie + Belohnung
                // Darf das Maximum nicht überschreiten
                const newEnergy = Math.min(maxEnergy, realEnergy + reward.rewardAmount);
                
                // Datenbank Update
                await updateUser(user.id, { 
                    energy: newEnergy, 
                    lastEnergyUpdate: Date.now() // WICHTIG: Timer zurücksetzen auf JETZT
                });
    ...
    const startBattle = () => {
        ...
        // 1. Berechne die ECHTE Energie (DB-Wert + Zeit-Regeneration)
        const realEnergy = calculateCurrentEnergy(user);
        ...
        // Check mit realer Energie
        if (realEnergy < 1) { showNotification("Keine Energie!", 'error'); return; }
        
        // 2. Energie abziehen und DB aktualisieren
        // Hier "bezahlen" wir für den Kampf und speichern den neuen Zeitstempel
        // Damit wird die "schwebende" regenerierte Energie fixiert.
        updateUser(user.id, { 
            energy: realEnergy - 1, 
            lastEnergyUpdate: Date.now() // Reset des Timers auf JETZT
        ...
        idsToLevel.forEach(petId => {
            ...
            if(pet) {
                ...
                if (pXp >= pMaxXp) {
                    ...
                    const rarityKey = pet.rarity || 'COMMON'; const rarityInfo = RARITIES[rarityKey] || RARITIES.COMMON; const rId = rarityInfo.id; const getBoost = () => Math.floor(Math.random() * 2) + rId; const newMaxHp = (pet.maxHp || 10) + getBoost() + 2; 
...
const handleLose = async (enemyRating) => {
        if (!user) return;
        const targetRating = enemyRating || user.rating || 1000;
        const eloChange = calculateEloChange(user.rating || 1000, targetRating, false); // Gibt negative Zahl zurück
        
        // Daily Elo Logic
        const today = new Date().toLocaleDateString();
        const isNewDay = user.lastEloDate !== today;
        const currentDailyChange = isNewDay ? 0 : (user.dailyEloChange || 0);
        const newDailyChange = currentDailyChange + eloChange; // eloChange ist negativ, also wird abgezogen
        ...
        await updateUser(user.id, {
            ...
            xp: newXp, level: newLevel, xpToNextLevel: newXpToNext, coins: newCoins, gems: newGems, energy: newEnergy,
            // Elo History speichern
            dailyEloChange: newDailyChange,
            lastEloDate: today,
...
const handleBuyMarket = async (listingId) => {
        ...
        // Wir rufen die sichere Transaktion in db.js auf
        const result = await buyMarketItem(user, listingId);

        if (result.success) {
            showNotification(result.message, 'success');
            // Kein manuelles Update nötig, Firestore Listener machen das
            trackQuestProgress(user, QUEST_TYPES.SPEND_COINS, 1); // Optional: Tracking wenn Preis bekannt wäre
        } else {
...
const handleSellMarket = async (petsToSell, totalPrice) => {
        ...
        // Fallback: Falls nur ein einzelnes Pet-Objekt (statt Array) übergeben wurde, pack es in ein Array
        ...
        const newListing = { 
            ...
            sellerId: user.id, 
            price: totalPrice, 
            // Für die Anzeige speichern wir das erste Pet direkt im 'pet' Feld
            pet: petArray[0], 
            // Für die Transaktion speichern wir ALLE im 'pets' Feld
            pets: petArray,
        ...
        // Alle Pets aus der eigenen Sammlung entfernen

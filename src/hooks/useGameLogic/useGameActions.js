...
export function useGameActions(state, setUserId) { 
    ...
    const handleWin = async (reward, winningTeamIds, enemyRating) => {
        ...
        // FIX: Gegner Rating ist immer gleich dem User Rating (oder 1000), falls nicht anders angegeben.
        // Das garantiert ~16 Elo Gewinn.
            ...
            if(pet) {
                ...
                if (pXp >= pMaxXp) {
                    ...
                    changes = {
                        ...
                        atk: (pet.atk || 1) + getBoost(),
                        ap: (pet.ap || 1) + getBoost(),
                        def: (pet.def || 1) + getBoost(),
                        res: (pet.res || 1) + getBoost(),
                        speed: (pet.speed || 1) + getBoost()
                    };
                } else {
                    changes = { xp: pXp };
                }
    ...
    const handleLose = async (enemyRating) => {
        ...
        // FIX: Auch hier dynamisches Rating für fairen Abzug
        const targetRating = enemyRating || user.rating || 1000;

        const eloChange = calculateEloChange(user.rating || 1000, targetRating, false);
        
        const lossReward = { xp: 10, coins: 5 }; 
        const petXpGain = 10; 

        let newLevel = user.level || 1;
        let newXp = (user.xp || 0) + lossReward.xp;
        let newXpToNext = user.xpToNextLevel || 100; 
        let newCoins = (user.coins || 0) + lossReward.coins;
        let newGems = user.gems || 0;
        let newEnergy = user.energy || 0;
        ...
        await updateUser(user.id, {
            ...
            coins: newCoins, gems: newGems, energy: newEnergy,

import { RARITIES, QUEST_TYPES, SHOP_ITEMS, TYPES, SECRET_ANIMALS, LOOTBOXES } from '../../data/gameData'; 
import { 
    generatePet, 
    calculateEloChange, 
...
export function useGameActions(state, setUserId) { 
    ...
    const handleRedeemTicket = async (ticketId) => {
        console.warn("Tickets werden jetzt direkt verwendet.");
    };
    ...
    const handleReduceCooldown = async (petId, type) => {
        ...
        if (!ticketItem) {
            showNotification("Keine Tickets im Inventar!", "error");
            return;
        }
        ...
        if (type === 'BREEDING') {
            if (!pet.bredAt || pet.bredAt + RARITIES[pet.rarity].breedCooldown <= Date.now()) {
                showNotification("Dieses Pet hat keinen aktiven Cooldown.", "error");
                return;
            }
            updates = { bredAt: pet.bredAt - FIVE_MINUTES };
            didReduce = true;
        } else if (type === 'HATCHING') {
            if (!pet.hatchAt || pet.hatchAt <= Date.now()) {
                showNotification("Dieses Ei ist bereits bereit!", "error");
                return;
            }
    ...
    return {
        ...
        buyLootbox, buyTickets, handleRedeemTicket, handleReduceCooldown,

...
export const getAdminAnalytics = async () => {
    ...
    try {
        ...
        leaderboardSnap.forEach(doc => {
            const d = doc.data();
            totalBattles += (d.stats?.pvpTotal || 0);
            totalWins += (d.stats?.pvpWins || 0);
...
export const sendFriendRequest = async (fromUser, toUserId) => {
    if (!fromUser || !toUserId || fromUser.id === toUserId) return { success: false, reason: 'invalid' };
    const toUserRef = doc(db, "users", toUserId);
    try {
        const toUserSnap = await getDoc(toUserRef);
        if (!toUserSnap.exists()) return { success: false, reason: 'not_found' };
        const toUserData = toUserSnap.data();

        if ((toUserData.friends || []).some(f => f.id === fromUser.id))
            return { success: false, reason: 'already_friends' };
        if ((toUserData.friendRequests || []).some(r => r.fromId === fromUser.id))
        ...
        await updateDoc(toUserRef, { friendRequests: arrayUnion(request) });
        return { success: true, targetUsername: toUserData.username };
    } catch (e) {

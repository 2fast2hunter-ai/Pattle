...
import { calculatePetLevelFromXp, recalculatePetStats, calculateMaxXp } from './mechanics/petStats'; // NEU
...
// NEU: Pet Migration Helper
const checkAndMigratePet = async (pet) => {
    if (pet.isEgg) return; // Eier haben kein Level

    const correctLevel = calculatePetLevelFromXp(pet.xp || 0, pet.rarity);
    // Prüfen ob Level falsch ist ODER ob die maxXp Hürde veraltet ist
    const correctMaxXp = calculateMaxXp(correctLevel, pet.rarity);

    // Wir erlauben eine kleine Toleranz oder prüfen strikt
    if (pet.level !== correctLevel || pet.maxXp !== correctMaxXp) {
        console.log(`[DB] Migriere Pet ${pet.name}: Lvl ${pet.level} -> ${correctLevel}`);
        
        // Stats komplett neu berechnen für das korrekte Level
        const newStats = recalculatePetStats(pet, correctLevel);
        
        try {
            await updateDoc(doc(db, "pets", pet.id), newStats);
...
export const listenToUser = (userId, callback) => { 
    return onSnapshot(doc(db, "users", userId), (docSnap) => { 
        if (docSnap.exists()) {
            ...
            checkAndMigrateLevel(userData); // Auto-Fix beim Laden
            callback(userData); 
        }
    }); 
};
...
// ... (Restlicher Code bleibt gleich wie im vorherigen Schritt) ...
